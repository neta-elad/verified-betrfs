...
(assert  (and (and (and (and (= (type $_Frame@0) (MapType4Type refType boolType)) (= (type $Heap) (MapType0Type refType MapType1Type))) (= (type |s3#0|) (MapType0Type BoxType boolType))) (= (type |s1#0|) (MapType0Type BoxType boolType))) (= (type |s2#0|) (MapType0Type BoxType boolType))))
(push 1)
(set-info :boogie-vc-id Impl$$Example.__default.EqualSets)
(set-option :timeout 0)
(set-option :rlimit 16350000)
(assert (not
 (=> (= (ControlFlow 0 0) 56434) (let ((anon0_correct  (=> (and (= $_Frame@0 (|lambda#1| null $Heap alloc false)) (= (ControlFlow 0 33813) (- 0 56637))) (= (U_2_bool (MapType0Select |s3#0| ($Box |s1#0|))) (U_2_bool (MapType0Select |s3#0| ($Box |s2#0|)))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is |s1#0| (TSet |#$Value|)) ($IsAlloc |s1#0| (TSet |#$Value|) $Heap)) (and ($Is |s2#0| (TSet |#$Value|)) ($IsAlloc |s2#0| (TSet |#$Value|) $Heap))) (=> (and (and (and ($Is |s3#0| (TSet (TSet |#$Value|))) ($IsAlloc |s3#0| (TSet (TSet |#$Value|)) $Heap)) (= 8 $FunctionContextHeight)) (and (forall ((|x#1| T@U) ) (!  (=> (and (= (type |x#1|) BoxType) ($IsBox |x#1| |#$Value|)) (= (U_2_bool (MapType0Select |s1#0| |x#1|)) (U_2_bool (MapType0Select |s2#0| |x#1|))))
 :qid |exampledfy.22:21|
 :skolemid |911|
 :pattern ( (MapType0Select |s2#0| |x#1|))
 :pattern ( (MapType0Select |s1#0| |x#1|))
)) (= (ControlFlow 0 56434) 33813))) anon0_correct)))))
PreconditionGeneratedEntry_correct)))
))
(check-sat)
(get-info :reason-unknown)
(get-value ((ControlFlow 0 0)))
(get-value ((ControlFlow 0 56434)))
(get-value ((ControlFlow 0 33813)))
(assert (not (= (ControlFlow 0 33813) (- 56637))))
(check-sat)
(pop 1)
; Invalid
(reset)
